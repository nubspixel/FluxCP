...
class Flux_Template {
	...
	{
		throw new Exception("This function 'itemTypeText' is now deprecated!");
	 ...
	 *
	 */
	public function equippableJobs($equipJob, $isRenewal=true)
	{
		$equipJobs = Flux::getEquipJobsList();
		if (is_array($equipJob)) {
			if (count($equipJob) == 1 && $equipJob[0] == 'job_all') {
				return array('All Jobs');
			}
			else {
				$data = array();
				foreach ($equipJob as $k => $v) {
					$index = str_replace('job_', '', $v);
					$data[] = $equipJobs[$index];
				}
				return $data;
			}
		}
		else {
			return "ERROR! EquippableJobs() unable to find any job";
		}
	}
	...
	{
		$equipClass   = Flux::getEquipUpperList();
		if (is_array($equipUpper)) {
			if (count($equipUpper) == 1 && $equipUpper[0] == 'class_all') {
				return array('All Class');
			}
			elseif (count($equipUpper) == 0) {
				return array('All Class');
			}
			else {
				$data = array();
				foreach ($equipUpper as $k => $v) {
					$index = str_replace('class_', '', $v);
					$data[] = $equipClass[$index];
				}
				return $data;
			}
		}
		else {
			return "ERROR! equipUpper() unable to find any job";
		}
	}
